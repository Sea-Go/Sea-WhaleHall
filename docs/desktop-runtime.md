...
This is the integration reference for WhaleHall's desktop processes, native
setup, Local Tool protocol, and local activity storage. Start with the
[quick start](../README.md#quick-start) to run the application, or use the
[documentation map](README.md) to find a feature guide.
Run the commands in this reference from the repository root.

- [Architecture](#architecture) and [development areas](#development-areas)
- [Platform prerequisites](#platform-prerequisites) and [macOS signing](#macos-development-signing)
- [Packaging and account migration](#packaging-and-account-migration)
- [Build and integration gates](#build-and-integration-gates)
- [Local Tool protocol](#local-tool-protocol) and [tool catalogue](#initial-local-tools)
- [Activity storage](#foreground-application-usage-and-sqlite) and [verification](#verifying-activity-history)
...
Application update control and macOS installation live under
[`src/bun/updates`](../src/bun/updates), with Bun controller, adapter, and native
installer tests under [`tests/bun/updates`](../tests/bun/updates).
...
## Packaging and account migration
...
DataCenter origin. The owner-editable `config.yaml` selects only the fixed logical
...
[`docs/REMOTE_MODEL_CONFIGURATION.md`](REMOTE_MODEL_CONFIGURATION.md) for
the configuration and deployment boundary, and
[`docs/CONVERSATION_AGENT_INTEGRATION.md`](CONVERSATION_AGENT_INTEGRATION.md)
