...
```mermaid
flowchart LR
  React["React WebViews\nUI and interaction only"] <-->|"Typed RPC"| Bun["Electrobun Bun main\nauth, encrypted storage, policy, calendar"]
  Bun <-->|"private Content-Length stdio"| Mastra["Bundled Node 22.18 Sidecar\nMastra conversation, planning, Tool loop"]
  Mastra -->|"complete model request via Bun"| Bun
  ...
  Bun -->|"JSONL stdio"| Rust["Rust Local Tool Host"]
  Rust --> Reflection["Local sensors and reflection pipeline"]
```
...
| TypeScript Agent boundary | [`src/agent`](src/agent) | Thin local process clients and handwritten protocol mirrors |
...
The pre-build hook compiles both Rust child processes, verifies and stages the pinned Node runtime, bundles the Mastra Sidecar, and builds the React views. Desktop authentication/model requests require `WHALEHALL_RELAY_URL` and `WHALEHALL_MODEL_ID`; provider keys belong only in the separately deployed relay process. See [`docs/CONVERSATION_AGENT_INTEGRATION.md`](docs/CONVERSATION_AGENT_INTEGRATION.md).
