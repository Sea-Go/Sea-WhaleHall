...
flowchart TB
  subgraph App["Electrobun application"]
    ...
    Bun <-->|"private Content-Length stdio v3"| Mastra["Bundled Node 22.18 Mastra Sidecar\nconversation, planning, workflows"]
    Mastra -->|"complete OpenAI-compatible request"| Bun
    Bun -.->|"HTTPS · bearer · purpose · Agent ID"| DataCenter["DataCenter data origin\nauth, model audit, Agent, cloud sync"]
...
Conversation history assembly, planning workflows, clarification, Tool policy and approval infrastructure, conflict validation, recovery, and local persistence run inside the desktop application. Interactive conversation is currently text-only: it registers no product Tools until the pinned production provider passes an OpenAI tool-call conformance gate, and raw Tool markup fails closed instead of becoming executable or visible text. Every remote model call, including sealed-window reflection and Dynamic Planning analysis, uses DataCenter's single `/v1/chat/completions` gateway with the current account's short-lived bearer. The Sidecar Agent Host binds each model call to a fixed catalog Agent ID and carries it over private stdio v3; Bun verifies the catalog entry, owning run, and purpose before adding `X-WhaleHall-Model-Agent` together with the code-owned `agent`, `activity`, `planning`, or `reflection` purpose. Activity runs additionally enforce one supervisor, one fixed-catalog specialist, and one voice call in that order; the Sidecar's guarded route mapping chooses which of the five catalog specialists is used, and a failed specialist open must retry that same Agent. DataCenter maps that Agent to a `high`, `medium`, or `low` reasoning tier. Renderer code, the request body, and caller-supplied HTTP headers cannot declare or override user, purpose, or Agent identity. DataCenter validates bearer identity, catalog membership, and purpose matching, but does not treat the Agent header as device or official-client attestation. Internal-test DataCenter deployments retain the exact request and response for developer review under the authenticated user. Before a WhaleHall release, DataCenter must already provide durable exact-response replay and user-scoped audit storage, and the pinned [DataCenter integration workflow](.github/workflows/datacenter-integration.yml) must pass for the exact WhaleHall candidate. Browser contexts never communicate directly, never receive bearer tokens or relay keys, and never supply account identity.
...
| Local Mastra Agent        | [`src/agent/mastra-host`](src/agent/mastra-host)             | Bundled Node ESM Sidecar, conversation/activity Agents, planning/reflection Workflows, and private stdio protocol v3 |
