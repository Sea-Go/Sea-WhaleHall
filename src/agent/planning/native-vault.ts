...
export const PLANNING_VAULT_REFERENCE_SCHEMA =
	"planning.runtime.reference.v1";
...
export async function sealPlanningPlan(
	...
	if (!boundedString(operationId, 1, 200) || !operationId.trim()) {
	...
	const chunkCount = Math.max(1, Math.ceil(payload.byteLength / RAW_CHUNK_BYTES));
...
function chunkRecordId(value: unknown): value is string {
	return typeof value === "string" && /^planning-chunk-[a-f0-9]{64}$/.test(value);
}
