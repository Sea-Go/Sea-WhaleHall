...
export class PlanningVaultGarbageCollector {
		...
		try {
			for (const reference of references) {
				if (reference.manifestRecordId === null) {
					throw new Error(
						"Planning Vault reference omitted its manifest identity.",
					);
				}
		...
		for (let pageIndex = 0; pageIndex < MAX_SCAN_PAGES; pageIndex += 1) {
			...
			for (const record of page.records) {
				if (
					...
					contentRefs.has(record.contentRef)
				) {
	...
	private async scanPlanningReferences(): Promise<
		PlanningVaultReferenceRecord[]
	> {
		...
		for (let pageIndex = 0; pageIndex < MAX_SCAN_PAGES; pageIndex += 1) {
			...
			for (const reference of page.references) {
				...
				if (prior !== undefined && prior !== identity) {
					throw new Error(
						"Planning Vault reference identity was inconsistent.",
					);
				}
				if (prior === undefined) {
...
function isReferenceRecord(
	value: unknown,
): value is PlanningVaultReferenceRecord {
	return (
		isExactRecord(value, [
			"source",
			"planId",
...
function validCursor(value: unknown): value is string | null {
	return value === null || boundedString(value, 1, 1_024);
}
