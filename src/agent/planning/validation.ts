...
export function isPlanningPlan(value: unknown): value is PlanningPlan {
	...
	if (
		tasks.some((task) => task.dependencyTaskIds.some((id) => !taskIds.has(id)))
	) {
		return false;
	}
	if (
		!estimates.every(isPlanEstimate) ||
		!unique(estimates.map((item) => item.id))
	) {
		return false;
	}
	...
	if (
		(value.activeRevisionId !== null &&
			!revisionIds.has(value.activeRevisionId)) ||
		(value.proposedRevisionId !== null &&
			!revisionIds.has(value.proposedRevisionId)) ||
		revisions.some(
			(revision) =>
...
function isPlanRevision(
	...
	if (
		...
		).includes(value.trigger as never) ||
		!boundedText(value.goal, 1, 1_000) ||
		...
		!isRecordWithKeys(value.scheduleWindow, [
			"startDate",
			"endDateExclusive",
		]) ||
		...
		dayDistance(
			value.scheduleWindow.startDate,
			value.scheduleWindow.endDateExclusive,
		) !== 7 ||
		!validInstant(value.createdAt)
	) {
		return false;
...
function isScheduleItem(
	...
	return (
		compareDates(date, windowStart) >= 0 && compareDates(date, windowEnd) < 0
	);
}
...
function identifier(value: unknown): value is string {
	return (
		...
		value.length > 0 &&
		value.length <= 240 &&
		Array.from(value).every((character) => character.charCodeAt(0) >= 0x20)
	);
}
