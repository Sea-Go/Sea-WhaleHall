...
export function buildDeterministicSevenDaySchedule(
	...
	if (
		compareDates(request.window.endDateExclusive, request.window.startDate) <=
			0 ||
		...
		throw new Error(
			"Planning schedule window must contain exactly seven dates.",
		);
	}
	...
	for (const event of request.busyEvents) {
		...
		allocatedByTask.set(event.sourceTaskId, {
			minutes:
				(current?.minutes ?? 0) +
				Math.max(0, durationMinutes(event.start, event.end)),
			lastEnd:
	...
	const alreadyAllocated = request.busyEvents
		...
		.reduce(
			(total, event) =>
				total + Math.max(0, durationMinutes(event.start, event.end)),
			0,
		);
	...
	for (const task of pendingTasks) {
		...
		if (
			existingAllocation &&
			(earliest === null ||
				compareInstants(earliest, existingAllocation.lastEnd) < 0)
		) {
	...
	for (
		...
		for (const preference of preferences) {
			...
			if (compareInstants(cursor, now.toString()) < 0) {
				const rounded =
					Math.ceil(now.epochMilliseconds / (15 * 60_000)) * 15 * 60_000;
			...
			const relevantBusy = busy
				.filter((item) =>
					intervalsOverlap(cursor, windowEnd, item.start, item.end),
				)
			...
			for (const occupied of relevantBusy) {
				...
				if (earliest && compareInstants(cursor, earliest) < 0)
					cursor = earliest;
			}
			const candidateEnd = addInstantMinutes(cursor, duration);
