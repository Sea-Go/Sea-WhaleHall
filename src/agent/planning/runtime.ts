...
export class PlanningRuntime {
	...
	async recoverPendingAdjustments(): Promise<readonly PlanningPlan[]> {
		...
		for (const snapshot of await this.repository.listPlans()) {
			const pending = snapshot.adjustments.find((item) => item.status === "pending");
			if (!pending) continue;
	...
	async createPlanDraft(request: CreatePlanDraftRequest): Promise<PlanningPlan> {
		assertOperationId(request.operationId);
		const replay = await this.repository.getOperationResult(request.operationId);
		...
		return this.analyzePlan(staged, "initial-analysis", request.operationId, false);
	}

	async sendPlanMessage(request: SendPlanMessageRequest): Promise<PlanningPlan> {
		assertOperationId(request.operationId);
		const replay = await this.repository.getOperationResult(request.operationId);
		if (replay) return replay;
		let staged = await this.repository.getOperationResult(
		...
		if (!staged) {
			const plan = await this.requireVersion(request.planId, request.expectedVersion);
			if (plan.status === "archived") {
				throw new PlanStateError("invalid-state", "Archived plans cannot receive messages.");
			}
	...
	async confirmPlanRevision(
		...
		if (pending) return this.finalizePendingAdjustment(pending, request.operationId);
		const plan = await this.requireVersion(request.planId, request.expectedVersion);
		if (
		...
		const proposal = plan.revisions.find((item) => item.id === request.revisionId);
		if (!proposal) {
			throw new PlanStateError("revision-not-found", "Planning revision was not found.");
		}
		...
		return this.prepareAndApplyAdjustment(
			{
				...confirmationPlan,
				effectiveStartDate:
					confirmationPlan.effectiveStartDate ?? startDate,
			},
			prepared,
			request.operationId,
			"manual-confirmation",
	...
	async setTaskStatus(request: SetTaskStatusRequest): Promise<PlanningPlan> {
		...
		if (!staged) {
			...
			if (!task) throw new PlanStateError("task-not-found", "Planning task was not found.");
			const now = this.now();
			staged = await this.saveNext(
				{
					...plan,
			...
			for (const summary of summaries) {
				...
				const candidates = summary.candidates.filter(
					(candidate) =>
						...
						activeTaskIds.has(candidate.taskId),
				);
				...
				if (
					...
					evidence.push(
						this.observationEvidence(plan.id, summary, unique.taskId, unique.confidence, "unique-observed"),
					);
					accepted = true;
				} else {
					pending.push({
						...
						status:
							!summary.authorized || summary.coverage !== "complete"
								...
								: candidates.some(
										(candidate) =>
											candidate.confidence < this.minimumObservationConfidence,
									)
	...
	async confirmObservationAttribution(
		...
		if (!staged) {
			...
			if (!activeRevisionTaskIds(plan).has(request.taskId)) {
				throw new PlanStateError("task-not-found", "Planning task was not found.");
			}
	...
	async resumePlan(request: ChangePlanStatusRequest): Promise<PlanningPlan> {
		...
		if (!staged) {
			...
			if (plan.status !== "paused") {
				throw new PlanStateError("invalid-state", "Only paused plans can be resumed.");
			}
			staged = await this.saveNext(
				{
					...plan,
	...
	async archivePlan(request: ChangePlanStatusRequest): Promise<PlanningPlan> {
		return this.changeStatus(request, "archived", ["draft", "awaiting-confirmation", "completed"]);
	}
	...
	async undoPlanAdjustment(
		...
		const adjustmentVersion =
			plan.adjustments.findIndex((item) => item.id === request.adjustmentId) + 1;
		if (!adjustment || adjustment.status !== "applied") {
			throw new PlanStateError(
		...
		const previous = adjustment.previousRevisionId
			? plan.revisions.find((item) => item.id === adjustment.previousRevisionId) ?? null
			: null;
		const now = this.now();
		return this.saveNext(
			{
	...
	private async analyzePlan(
		...
		try {
			...
			try {
				const useActiveBaseline = automatic || options.forceActiveBaseline === true;
	...
	private async materializeModelProposal(
		...
		const resolvedType = automatic && plan.type ? plan.type : output.recommendedType;
		if (resolvedType === "fuzzy" && output.confidence > 0.5) {
			throw new PlanningModelOutputError();
		}
		const goal = automatic ? plan.goal : validUserText(output.goal, "goal", 1_000);
		const mapped = mapModelTasks(plan, output.tasks, this.createId, automatic);
		const window = rollingSevenDayWindow(startDate);
		const mutableStartDate = automatic
			? laterDate(addDays(localDateAt(this.clock.nowMs(), plan.timeZone), 1), startDate)
			: startDate;
	...
	private async rematerializeRevision(
		...
		if (!estimate) throw new PlanStateError("invalid-state", "Revision estimate is missing.");
		return {
			revision,
			estimate,
	...
	private async prepareAndApplyAdjustment(
		...
		if (!manualConfirmation && prepared.revision.unscheduledTaskIds.length > 0) {
		...
		return this.saveNext(
			{
				...
				proposedRevisionId: manualConfirmation
					? null
	...
	private async finalizePendingAdjustment(
		...
		const adjustment = [...pending.adjustments]
			.reverse()
			.find((item) => item.operationId === operationId && item.status === "pending");
		if (!adjustment) {
			throw new PlanStateError("invalid-state", "Pending calendar adjustment is missing.");
		}
		const result = await this.calendar.applyChangeSet(adjustment.calendarChangeSet);
		const now = this.now();
		const nextRevision = pending.revisions.find(
			(item) => item.id === adjustment.nextRevisionId,
		);
		if (!nextRevision) {
			throw new PlanStateError("revision-not-found", "Adjustment revision is missing.");
		}
		if (!result.ok) {
			const failed = {
				...pending,
...
function mapModelTasks(
	...
	for (const task of modelTasks) {
		idBySourceKey.set(
			task.taskKey,
			existingBySourceKey.get(task.taskKey)?.id ?? prefixedId("task", createId()),
		);
	}
	const projectedBySourceKey = new Map(modelTasks.map((task) => [task.taskKey, task]));
	const projectTask = (task: PlanningModelProposal["tasks"][number]): PlanTask => {
	...
	for (const modelTask of modelTasks) {
		if (!existingSourceKeys.has(modelTask.taskKey)) tasks.push(projectTask(modelTask));
	}
	...
	const revisionTasks = activeTasks.map<RevisionTask>(
		(task) =>
...
function requireMappedId(ids: ReadonlyMap<string, string>, key: string): string {
...
function confirmationNeedsReview(
	...
	const scheduledTaskIds = new Set(refreshed.schedule.map((item) => item.taskId));
	return (
		!refreshed.tasks.some(
			(task) => task.purpose === "validation" && scheduledTaskIds.has(task.taskId),
		) ||
		!refreshed.tasks.some(
...
function reverseChangeSet(
	...
	const changes = [...changeSet.changes].reverse().map<CalendarEventMutation>((change) => {
		...
		throw new PlanStateError("invalid-state", "Adjustment cannot be reversed.");
	});
	return {
		id,
...
function currentGoalForAnalysis(plan: PlanningPlan, automatic: boolean): string {
...
function activeRevision(plan: PlanningPlan): PlanRevision | null {
	...
	return (
		plan.revisions.find((revision) => revision.id === plan.activeRevisionId) ?? null
	);
}

function activeRevisionTaskIds(plan: PlanningPlan): ReadonlySet<string> {
	return new Set((activeRevision(plan)?.tasks ?? []).map((task) => task.taskId));
}
...
function deriveShortTermEstimate(
	...
	const remainingMinutes = pending.reduce(
		(total, task) =>
			...
			Math.max(
				0,
				task.estimatedMinutes - (observedByTask.get(task.id) ?? 0),
			),
		0,
	);
...
function prefixedId(prefix: string, id: string): string {
	if (!id.trim()) throw new Error("Planning ID generator returned an empty ID.");
	return `${prefix}_${id}`;
}

function assertOperationId(operationId: string): void {
	if (!operationId.trim() || operationId.length > 200) {
		throw new PlanStateError("invalid-input", "A stable operation ID is required.");
	}
}
