...
export class WhaleHallPlanningRuntime {
	...
	async setTaskStatus(command: Parameters<PlanningRuntime["setTaskStatus"]>[0]) {
		return this.afterWrite(
	...
	private async projectPlan(plan: PlanningPlan): Promise<PlanningPlanProjection> {
		...
		const calendarById = new Map(calendar.events.map((event) => [event.eventId, event]));
		...
		const activeTaskIds = new Set(active?.tasks.map((task) => task.taskId) ?? []);
		...
		const revisionProjection = (
			...
			const taskStatusById = new Map(plan.tasks.map((task) => [task.id, task.status]));
			...
			return {
				...
				tasks: revision.tasks.map((task) => ({
					...
					description: task.description,
					purpose: taskPurpose(task),
					status: isActive ? (taskStatusById.get(task.taskId) ?? "pending") : "pending",
					estimatedMinutes: task.estimatedMinutes,
					dependsOnTaskIds: [...task.dependencyTaskIds],
					schedules: isActive
		...
		for (const summary of await this.runtime.listPlans()) {
			...
			for (let attempt = 0; attempt < 2; attempt += 1) {
				try {
					...
					const latest = await this.runtime.getPlan(current.id).catch(() => null);
					if (
						!latest ||
...
class VerifiedQwenPlanningModel implements PlanningModelPort {
	readonly modelVersion = `${WHALEHALL_TEACHER_MODEL_LOCK.model}@${WHALEHALL_TEACHER_MODEL_LOCK.digest.slice(0, 12)}`;
	private model: QwenPlanningModel | null = null;
	private verification: Promise<QwenPlanningModel> | null = null;

	async analyze(request: PlanningModelAnalysisRequest): Promise<PlanningModelOutput> {
...
function taskPurpose(task: unknown) {
	const purpose =
		task && typeof task === "object" && "purpose" in task
			? task.purpose
			: null;
...
function projectPersistentNotifications(plan: PlanningPlan) {
	...
	for (const adjustment of plan.adjustments) {
		...
		const previousEstimate = estimateDateForRevision(plan, adjustment.previousRevisionId);
		const nextEstimate = estimateDateForRevision(plan, adjustment.nextRevisionId);
...
function estimateDateForRevision(plan: PlanningPlan, revisionId: string | null) {
...
function projectionRevisionReason(trigger: PlanningPlan["revisions"][number]["trigger"]) {
	switch (trigger) {
		case "initial-analysis": return "initial-analysis" as const;
		case "conversation": return "conversation" as const;
		case "task-status": return "user-progress" as const;
		case "observation": return "observation" as const;
		case "calendar-change": return "calendar-change" as const;
		case "daily-summary": return "daily-summary" as const;
		case "confirmation":
		case "resume":
...
function projectAdjustments(plan: PlanningPlan): PlanningAdjustmentProjection[] {
	return plan.adjustments.map((adjustment, index) => {
		const previous = adjustment.previousRevisionId
			...
			: null;
		const next = plan.revisions.find((item) => item.id === adjustment.nextRevisionId);
		...
		const nextEstimate = next
			...
			: null;
		const laterApplied = plan.adjustments.slice(index + 1).some((item) => item.status === "applied");
		...
		return {
			...
			canUndo,
			undoUnavailableReason: canUndo ? null : "只有最新且未被后续版本覆盖的调整可以撤销。",
