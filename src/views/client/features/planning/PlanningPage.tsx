	...
	if (state.status === "model-unavailable" && state.content) {
		return (
			<InlineNotice tone="warning" icon={<Bot size={17} />}>
				...
				{state.retryable ? <Button size="small" onClick={onRetry}>重试分析</Button> : null}
			</InlineNotice>
		);
	}
	if (state.status === "stale") {
		return (
			<InlineNotice tone="warning" icon={<RefreshCw size={17} />}>
				<span>{state.message}</span>
				<Button size="small" onClick={onRetry}>载入最新版本</Button>
			</InlineNotice>
		);
	}
	if (state.status === "offline" && state.cached) {
		return (
			<InlineNotice tone="warning" icon={<WifiOff size={17} />}>
				<span>{state.message}</span>
				{state.retryable ? <Button size="small" onClick={onRetry}>重新连接</Button> : null}
			</InlineNotice>
		);
	}
	if (state.status === "error" && state.cached) {
		return (
			<InlineNotice tone="error" icon={<AlertTriangle size={17} />}>
				<span>{state.message}</span>
				{state.retryable ? <Button size="small" onClick={onRetry}>重试</Button> : null}
			</InlineNotice>
		);
	}
	return null;
	...
	return (
		<section className="planning-page" aria-label="动态计划">
			<PageHeader
				...
				action={
					content && state.status !== "create" && state.status !== "creating" ? (
						<Button
							variant="primary"
			...
			<form className="planning-create-form" onSubmit={handleSubmit}>
				<div className="planning-field">
					...
					<textarea
						...
						aria-describedby={issue ? "planning-goal-error" : "planning-goal-help"}
						onChange={(event) =>
					...
					{issue ? (
						...
						<small id="planning-goal-help">后续可以随时通过对话修正目标。</small>
					)}
				</div>
...
function PersistentNotifications({ plan }: { plan: PlanView }) {
	...
	return (
		<section className="planning-persistent-notifications" aria-label="计划通知">
			{plan.notifications.map((notification) => (
				<InlineNotice
					key={notification.id}
					tone={notification.kind === "attention-required" ? "warning" : "info"}
					icon={notification.kind === "attention-required"
	...
	return (
		<header className="planning-overview">
			...
			<div className="planning-overview__metrics">
				...
				<div>
					<span>动态预计完成</span>
					<strong>{formatDate(plan.estimate?.estimatedCompletionDate ?? null)}</strong>
					<small>
						{plan.estimate
...
function RevisionPanel({
	...
	return (
		<section className="planning-card planning-revision" aria-labelledby="revision-title">
			<div className="planning-panel-heading planning-panel-heading--row">
				<div className="planning-panel-heading__icon"><Sparkles size={17} /></div>
				<div>
					<h2 id="revision-title">
						{revision.status === "proposed" ? "模型建议" : "当前计划版本"}
					</h2>
					<p>修订 {revision.version} · {typeLabels[revision.planType]}</p>
				</div>
				<span className={`planning-confidence planning-confidence--${revision.estimate.confidence}`}>
			...
			<div className="planning-revision__eta">
				...
				<div>
					...
					<strong>
						{formatDate(revision.scheduleWindow.startDate)} – {formatDate(revision.scheduleWindow.endDateInclusive)}
					</strong>
				</div>
			</div>
			...
			<div className="planning-compact-list planning-revision__preferences">
				...
				<p>
					每周 {formatMinutes(revision.schedulingPreferences.weeklyCapacityMinutes)}
					{" · "}单次 {formatMinutes(revision.schedulingPreferences.sessionMinutes)}
				</p>
				{revision.schedulingPreferences.availableWindows.length > 0 ? (
					<ul>
						{revision.schedulingPreferences.availableWindows.map((window, index) => (
							<li key={`${window.dayOfWeek}-${window.startTime}-${window.endTime}-${index}`}>
								{weekdayLabels[window.dayOfWeek]} {window.startTime}–{window.endTime}
							</li>
						))}
					</ul>
				) : (
			...
			{revision.assumptions.length > 0 ? (
				<div className="planning-compact-list">
					<strong>当前假设</strong>
					<ul>{revision.assumptions.map((item) => <li key={item}>{item}</li>)}</ul>
				</div>
			) : null}
			{revision.questions.length > 0 ? (
				<div className="planning-compact-list planning-compact-list--questions">
					<strong>需要你补充</strong>
					<ul>{revision.questions.map((item) => <li key={item}>{item}</li>)}</ul>
				</div>
			) : null}
			{revision.status === "proposed" ? (
				<div className="planning-revision__tasks">
					...
					{revision.tasks.length > 0 ? (
						...
						<p className="planning-card__empty">这版提案还没有可确认的七天任务。</p>
					)}
				</div>
			) : null}
			{revision.status === "proposed" ? (
...
function TaskPanel({
	...
	return (
		<section className="planning-card" aria-labelledby="tasks-title">
			<div className="planning-panel-heading planning-panel-heading--row">
				<div className="planning-panel-heading__icon"><ListTodo size={17} /></div>
				<div>
			...
			{plan.tasks.length === 0 ? (
				<p className="planning-card__empty">继续对话后，模型会在信息充分时给出任务建议。</p>
			) : (
				<ul className="planning-task-list">
					{plan.tasks.map((task) => (
						<TaskRow
							...
							onStatus={(status) => void controller.setTaskStatus(task.id, status)}
						/>
					))}
				</ul>
			)}
	...
	return (
		<li className={`planning-task planning-task--${task.status}`}>
			<div className="planning-task__state" aria-hidden="true">
				{task.status === "completed" ? <Check size={15} /> : <CircleDashed size={15} />}
			</div>
					...
					{task.schedules.map((schedule, index) => (
						<span className="planning-task__session" key={schedule.eventId}>
							<span>
								...
								第 {index + 1} 次 · {formatSession(schedule)}
							</span>
							{schedule.scheduleOrigin === "user" ? <span>用户安排</span> : null}
							{schedule.userLocked ? (
								<span><LockKeyhole size={13} />用户已锁定</span>
							) : null}
						</span>
					))}
				</div>
			...
			{readOnly ? null : <div className="planning-task__actions">
				{task.status === "pending" ? (
					<>
						<Button size="small" disabled={disabled} onClick={() => onStatus("completed")}>
							完成
						</Button>
						<Button variant="ghost" size="small" disabled={disabled} onClick={() => onStatus("skipped")}>
							跳过
						</Button>
					</>
				) : (
					<Button variant="ghost" size="small" disabled={disabled} onClick={() => onStatus("pending")}>
						重新打开
					</Button>
				)}
			</div>}
...
function ObservationPanel({
	...
	return (
		<section className="planning-card" aria-labelledby="observations-title">
			<div className="planning-panel-heading planning-panel-heading--row">
				<div className="planning-panel-heading__icon"><Target size={17} /></div>
				<div>
			...
			{!plan.monitoring.authorized || !plan.monitoring.enabled ? (
				...
				<p className="planning-observation-ready">观测已授权；歧义活动仍需你确认后才计入计划。</p>
			)}
			{pending.length > 0 ? (
				<div className="planning-observation-list">
					...
					{pending.map((observation) => (
						<ObservationRow
							...
							disabled={disabled}
							onConfirm={(taskId) =>
								void controller.confirmObservationAttribution(observation.id, taskId)
							}
						/>
					))}
				</div>
	...
	return (
		<article className="planning-observation">
			<div>
				...
				<p>{formatMinutes(observation.durationMinutes)} · {confidenceLabels[observation.confidence]}</p>
			</div>
			<div className="planning-observation__actions" aria-label="确认观测归因">
				{candidates.map((task) => (
					<Button key={task.id} size="small" disabled={disabled} onClick={() => onConfirm(task.id)}>
						计入“{task.title}”
					</Button>
				))}
				<Button variant="ghost" size="small" disabled={disabled} onClick={() => onConfirm(null)}>
					不计入计划
				</Button>
			</div>
		</article>
...
function AdjustmentPanel({
	...
	return (
		<section className="planning-card" aria-labelledby="adjustments-title">
			<div className="planning-panel-heading planning-panel-heading--row">
				<div className="planning-panel-heading__icon"><History size={17} /></div>
				<div>
				...
				{adjustments.map((adjustment) => (
					<li key={adjustment.id}>
						<div>
							...
							<p>
								预计完成：{formatDate(adjustment.previousEstimatedCompletionDate)} → {formatDate(adjustment.nextEstimatedCompletionDate)}
							</p>
							<small>
								移动 {adjustment.movedCount} · 新增 {adjustment.addedCount} · 取消 {adjustment.cancelledCount}
							</small>
						</div>
						{adjustment.canUndo ? (
							<Button
							...
							<span className="planning-adjustments__locked">
								{adjustment.undoneAt ? "已撤销" : adjustment.undoUnavailableReason ?? "不可撤销"}
							</span>
						)}
					</li>
				))}
...
function ConversationPanel({
	...
	const hasPending = plan.messages.some((item) => item.status === "pending-analysis");
	...
	return (
		<aside className="planning-conversation" aria-labelledby="conversation-title">
			<div className="planning-panel-heading planning-panel-heading--row">
				<div className="planning-panel-heading__icon"><MessageSquareText size={17} /></div>
				<div>
			...
			<div className="planning-messages" aria-live="polite">
				{plan.messages.map((item) => (
					<article key={item.id} className={`planning-message planning-message--${item.role}`}>
						<div>
