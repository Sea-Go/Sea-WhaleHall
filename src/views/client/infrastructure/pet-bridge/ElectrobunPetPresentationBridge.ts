	...
	implements PetPresentationBridge
{
	...
	async updateTodaySchedule(schedule: PetTodaySchedule): Promise<void> {
		if (!this.runtimeAvailable()) return;
		try {
			const transport = await this.loadTransport();
			await transport.updateTodaySchedule(schedule);
		} catch {
			this.logger.warn("[pet-bridge] schedule delivery failed", {
				operation: "update-today-schedule",
				category: "transport",
			});
		}
	}
...
async function loadClientTransport(): Promise<PetBridgeTransport> {
	...
	return {
		...
		async updateTodaySchedule(schedule) {
			await clientApi.updatePetTodaySchedule(schedule);
		},
	};
}
