...
export class ElectrobunPlanningService implements PlanningService {
		...
		void this.loadApi()
			.then((api) => {
				...
				stops.push(
					...
					api.onPlanningNotification((notification) =>
						listener({
	...
	async createPlanDraft(request: Parameters<PlanningService["createPlanDraft"]>[0]) {
		try {
			const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC";
			return (await this.loadApi()).createPlanDraft({ ...request, timeZone });
		} catch (error) {
			throw serviceError(error);
		}
	}

	async sendPlanMessage(request: Parameters<PlanningService["sendPlanMessage"]>[0]) {
		await this.write((api) => api.sendPlanMessage(request));
	}
	async confirmPlanRevision(request: Parameters<PlanningService["confirmPlanRevision"]>[0]) {
		await this.write((api) => api.confirmPlanRevision(request));
	}
	async setTaskStatus(request: Parameters<PlanningService["setTaskStatus"]>[0]) {
		await this.write((api) => api.setPlanningTaskStatus(request));
	}
	async confirmObservationAttribution(request: Parameters<PlanningService["confirmObservationAttribution"]>[0]) {
	...
	async undoPlanAdjustment(request: Parameters<PlanningService["undoPlanAdjustment"]>[0]) {
	...
	async retryPendingAnalysis(request: Parameters<PlanningService["retryPendingAnalysis"]>[0]) {
...
function fromTask(task: PlanningPlanProjection["tasks"][number]): PlanningTaskView {
	return {
		id: task.id,
		title: task.title,
...
function fromEstimate(value: PlanningPlanProjection["estimate"] & {}): PlanEstimate {
	return {
...
function fromObservation(value: PlanningPlanProjection["pendingObservations"][number]): PlanningObservationView {
	return {
		id: value.id,
...
function fromAdjustment(value: PlanningPlanProjection["adjustments"][number]): PlanAdjustmentView {
	return {
		id: value.id,
...
function serviceError(error: unknown): PlanningServiceError {
	...
	const message = error instanceof Error ? error.message : "Planning request failed.";
	...
	if (normalized.includes("not_found") || normalized.includes("was not found")) {
	...
	if (
		...
		return new PlanningServiceError("validation", message, { retryable: false });
	}
	if (
...
function invalidProjection(message: string, cause: unknown): PlanningServiceError {
