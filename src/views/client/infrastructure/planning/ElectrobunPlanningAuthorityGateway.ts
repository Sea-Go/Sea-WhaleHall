...
import type { PlanningAuthorityGateway } from "../../features/planning/planning-service";

export class ElectrobunPlanningAuthorityGateway
	implements PlanningAuthorityGateway
{
	...
	async saveDraft(
		...
		const { clientApi } = await import("../../rpc");
		return unwrap(
			await clientApi.savePlanningDraft({
	...
	async commitDraft(
		...
		const { clientApi } = await import("../../rpc");
		return unwrap(
			await clientApi.commitPlanningDraft({
				requestId: crypto.randomUUID(),
				commitId,
				expectedRevision,
...
export class PlanningAuthorityGatewayError extends Error {
	constructor(
		readonly kind: Exclude<
			PlanningAuthorityRpcResult<never>["kind"],
			"success"
		>,
		message: string,
	) {
