...
describe("ActivityEventWorkerClient", () => {
	test("sends the complete raw JSON payload with its dedicated bearer token", async () => {
		...
		if (capture.received === null)
			throw new Error("Worker request was not captured.");
	...
	test("cancels a rejected gateway response without retaining its body", async () => {
		let cancelled = false;
		const client = new ActivityEventWorkerClient({
			endpoint: "https://worker.example.test/v1/activity/analyze",
			authorizationToken: "dedicated-token",
		...
		await expect(
			client.analyze({
				schema_version: "activity-event-analysis-request.v1",
				request_id: "request-cancelled-error-body",
				raw_event: { eventId: "raw-cancelled-error-body" },
				context: {},
			}),
		).rejects.toMatchObject({ code: "http_error", httpStatus: 502 });
		expect(cancelled).toBeTrue();
	});

	test("rejects declared and streamed responses above the bounded read limit", async () => {
		const request = {
			schema_version: "activity-event-analysis-request.v1" as const,
			request_id: "request-large-response",
			raw_event: { eventId: "raw-large-response" },
			context: {},
		};
		const declaredLarge = new ActivityEventWorkerClient({
			endpoint: "https://worker.example.test/v1/activity/analyze",
			authorizationToken: "dedicated-token",
			fetch: async () =>
				new Response("{}", {
		...
		const streamedLarge = new ActivityEventWorkerClient({
			endpoint: "https://worker.example.test/v1/activity/analyze",
			authorizationToken: "dedicated-token",
			fetch: async () =>
				new Response(
		...
		await expect(declaredLarge.analyze(request)).rejects.toMatchObject({
			code: "invalid_response",
		});
		await expect(streamedLarge.analyze(request)).rejects.toMatchObject({
			code: "invalid_response",
		});
		expect(streamedBodyCancelled).toBeTrue();
	});
