...
class FakePlanningVaultMaintenancePort {
	...
	useLongInventoryCursors = false;
	useLongReferenceCursors = false;
	listRecordCalls = 0;
		...
		return {
			...
			nextCursor:
				nextOffset < eligible.length
					? cursorFor(nextOffset, this.useLongInventoryCursors)
					: null,
		};
	}
	...
	async listPlanningVaultReferences(
		...
		return {
			...
			nextCursor:
				nextOffset < this.references.length
					? cursorFor(nextOffset, this.useLongReferenceCursors)
					: null,
		};
	}
...
function cursorFor(offset: number, long: boolean): string {
	return long ? `${"a".repeat(600)}:${offset}` : String(offset);
}
...
describe("Planning Vault fail-closed garbage collection", () => {
	test("keeps current, history, and operation references while deleting only aged orphans", async () => {
		...
		const current = await sealReference(
			port,
			"plan-live",
			3,
			"live-current",
		...
		const history = await sealReference(
			port,
			"plan-live",
			2,
			"live-history",
		...
		const orphanRecordIds = new Set(
			[...port.records.keys()].filter(
				(recordId) => !beforeOrphan.has(recordId),
			),
		);
		...
		expect(port.records.has(`planning-manifest-${"a".repeat(64)}`)).toBeTrue();
	});
	...
	test("accepts complete opaque cursors large enough for maximum native IDs", async () => {
		const port = new FakePlanningVaultMaintenancePort();
		const current = await sealReference(
			port,
			"plan-long-cursor",
		...
		port.references = [current, history];
		port.useLongInventoryCursors = true;
		port.useLongReferenceCursors = true;
