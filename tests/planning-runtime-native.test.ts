...
class FakeNativePlanningAgent {
	readonly sealBatches: Array<
		Parameters<AgentRuntime["sealVaultBatch"]>[0]
	> = [];
	readonly openBatches: Array<
		Parameters<AgentRuntime["openVaultBatch"]>[0]
	> = [];
	...
	mutateNextOpen(
		mutation: (record: LocalVaultOpenResultRecord) => LocalVaultOpenResultRecord,
	): void {
	...
	async listPlanningPlans(
		...
		return { plans: [...this.plans.values()].map((plan) => structuredClone(plan)) };
	}
	...
	async listPlanningCalendar(): ReturnType<AgentRuntime["listPlanningCalendar"]> {
		return { events: [] };
	}
...
function largePlan(targetBytes: number): PlanningPlan {
	...
	const sampleBytes = Buffer.byteLength(JSON.stringify(makeMessage(0)), "utf8") + 1;
	...
	for (let index = initialCount; ; index += 1) {
		...
		if (Buffer.byteLength(JSON.stringify(candidate), "utf8") > targetBytes) break;
		messages.push(next);
	}
	const result = { ...plan, messages };
	if (!isPlanningPlan(result)) throw new Error("Large plan fixture is invalid.");
	return result;
}
...
describe("NativePlanningRepository encrypted persistence", () => {
	test("writes only safe indexes and restores the full plan through the vault", async () => {
		...
		expect(agent.sealBatches.every((batch) => batch.namespace === "planning.runtime.v1"))
			.toBeTrue();
		expect(agent.openBatches.every((batch) => batch.namespace === "planning.runtime.v1"))
			.toBeTrue();
	...
	test("supports a near-8 MiB plan with encrypted manifest and bounded chunks", async () => {
		...
		await expect(repository.createPlan(plan, "native-create-large")).resolves.toMatchObject({
			plan,
		});
		const chunkBatches = agent.sealBatches.filter(
			(batch) => batch.records[0]?.schemaVersion === "planning.runtime.chunk.v1",
		);
		expect(chunkBatches).toHaveLength(Math.ceil(bytes / (360 * 1024)));
		expect(agent.sealBatches.every((batch) => batch.records.length === 1)).toBeTrue();
	...
	test("strictly rejects reference, schema, identity, manifest, and ref mismatches", async () => {
		const cases: Array<{
			name: string;
			mutate(agent: FakeNativePlanningAgent, snapshot: LocalPlanningPlanSnapshot): void;
		}> = [
			{
				name: "namespace",
			...
			},
			{
				name: "plan identity",
				mutate(agent, snapshot) {
					agent.mutateVaultContent(String(snapshot.sealedContentRef), (content) => ({
		...
		for (const testCase of cases) {
			...
			const repository = new NativePlanningRepository(agent);
			const plan = activePlan();
			await repository.createPlan(plan, `strict-${testCase.name}`);
			const snapshot = agent.storedPlan(plan.id);
	...
	test("binds vault record IDs to operation and rejects same-operation content reuse", async () => {
		...
		const plan = activePlan();
		await new NativePlanningRepository(firstAgent).createPlan(plan, "operation-a");
		await new NativePlanningRepository(secondAgent).createPlan(plan, "operation-b");
		...
		await expect(
			new NativePlanningRepository(firstAgent).createPlan(changed, "operation-a"),
	...
	test("redacts model calendar titles before native persistence", async () => {
		...
		if (!mutation || mutation.action !== "upsert") {
