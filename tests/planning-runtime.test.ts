...
function harness(outputs: Array<PlanningModelOutput | Error>, clock?: FakeClock) {
	...
	return { runtime, repository, calendar, observations, model, clock: runtimeClock };
}
...
describe("PlanningRuntime adjustment rules", () => {
	...
	test("preserves a pending proposal through automatic progress and refreshes stale confirmation", async () => {
		...
		expect(adjusted.tasks.find((task) => task.id === completedTaskId)?.status).toBe(
			"completed",
		);
		...
		expect(confirmed.proposedRevisionId).toBeNull();
		expect(confirmed.tasks.find((task) => task.id === completedTaskId)?.status).toBe(
			"completed",
		);
	});
	...
	test("model omissions never delete stable tasks or explicit user status", async () => {
		...
		expect(completed.tasks.find((task) => task.id === omittedTask.id)).toMatchObject({
			status: "completed",
			statusChangedBy: "user",
		});
		expect(completed.revisions.at(-1)?.tasks.map((task) => task.taskId)).toContain(
			omittedTask.id,
		);
	});
	...
	test("keeps the last applied revision active when an automatic calendar batch fails", async () => {
		const app = await createAndConfirm("short-term", {
			outputs: [proposal("short-term"), proposal("short-term", { taskCount: 2 })],
		});
