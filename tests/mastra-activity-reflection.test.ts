...
function requestFixture(
	...
	return {
		...
		raw_event: {
			...
			events: [{ id: "raw-event-1", title: "private source text" }],
		},
		context: {
...
describe("MastraActivityReflectionAnalyzer", () => {
	test("delegates analysis and scoring instructions to Mastra-native filesystem Skills", () => {
		...
		expect(ACTIVITY_REFLECTION_SYSTEM_PROMPT).toContain("Mastra 原生 skill 工具");
	...
	test("requires a concrete Chinese action after its certainty prefix", () => {
		...
		expect(activityReflectionModelOutputSchema.safeParse(output).success).toBeFalse();
	});
	...
	test("rejects model-authored state events because state boundaries belong to the client", () => {
		...
		expect(activityReflectionModelOutputSchema.safeParse(output).success).toBeFalse();
		expect(() => activityReflectionOutputToWorkerResponse(output, requestFixture())).toThrow(
			"invalid_response",
		);
	});
	...
	test("retains a combined coding-and-research hypothesis and collapses overlapping or adjacent model guesses", () => {
		const output = {
			events: [
				{
					...modelOutputFixture().events[0]!,
					action: "确定：在使用Visual Studio Code",
		...
		const response = activityReflectionOutputToWorkerResponse(output, requestFixture());
	...
	test("adds deterministic zero-score state markers only for actual nonempty-window boundaries", () => {
		...
		request.raw_event = {
			...
			events: [
				{ kind: "editor.documentChanged", occurredAtMs: 1_100, sensitivity: "content" },
				{ kind: "presence.locked", occurredAtMs: 1_800, sensitivity: "metadata" },
			],
		};
		const output = modelOutputFixture();
		output.score = 0.6;
	...
	test("keeps a directly supported gaming example and a zero-score uncertain example reviewable", () => {
		...
		expect(
			activityReflectionOutputToWorkerResponse(gaming, requestFixture()).events[0]?.action,
		).toBe("确定：正在进行游戏");
		...
		expect(activityReflectionOutputToWorkerResponse(uncertain, requestFixture())).toMatchObject({
			events: [
				{
					action: "不确定：正在查阅技术资料",
					confidence: 0.3,
	...
	test("keeps the model score intact while redacting a sensitive score reason", () => {
		const output = modelOutputFixture();
		output.score = 0.6;
		output.score_reason = "目标直接相关，详见 https://private.invalid，计 0.60 分";
		const response = activityReflectionOutputToWorkerResponse(
			output,
			requestFixture(),
		);
	...
	test("builds the full client-owned prompt, then writes a local reviewable receipt", async () => {
		...
		const sidecar: ActivityReflectionSidecar = {
			async request<TResult = unknown>(
				...
				sidecarCalls.push({ method, params: structuredClone(params), ...options });
	...
	test("rejects malformed model output before it can reach the score ledger", async () => {
		...
		await expect(analyzer.analyze(requestFixture("invalid-model-output"))).rejects.toThrow(
			"invalid_response",
		);
	});
