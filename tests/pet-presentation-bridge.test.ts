...
describe("ElectrobunPetPresentationBridge", () => {
	test("delivers typed presentation and visibility calls through its transport", async () => {
		...
		const bridge = new ElectrobunPetPresentationBridge({
			...
			loadTransport: async () => ({
				...
				async updateTodaySchedule(schedule) {
					schedules.push(schedule.status);
				},
			}),
		});
		...
		await bridge.updateTodaySchedule({
			status: "ready",
			date: "2026-07-30",
		...
		expect(schedules).toEqual(["ready"]);
	});
	...
	test("silently degrades and records only sanitized diagnostic context", async () => {
		...
		const bridge = new ElectrobunPetPresentationBridge({
			...
			loadTransport: async () => ({
				...
				async updateTodaySchedule() {
					throw new Error("private schedule data");
				},
			}),
			logger: {
		...
		expect(diagnostics).toEqual([
			...
			{
				message: "[pet-bridge] schedule delivery failed",
				context: {
